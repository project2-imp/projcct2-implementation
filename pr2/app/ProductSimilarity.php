...
class ProductSimilarity
{
    ...
    {
        //catch products array
        $this->products       = $products;
        //get max price from products array
        $this->priceHighRange = max(array_column($products, 'priceForSeat'));
    }
    ...
    public function calculateSimilarityMatrix(): array
    {
        ...
        foreach ($this->products as $product) {
            ...
            foreach ($this->products as $_product) {
                if ($product->tripID === $_product->tripID) {
                    continue;
                }
                $similarityScores['product_id_' . $_product->tripID] = $this->calculateSimilarityScore($product, $_product);
            }
            $matrix['product_id_' . $product->tripID] = $similarityScores;
        }
        return $matrix;
    }
    ...
    public function getProductsSortedBySimularity(  int $productId, array $matrix): array
    {
        ...
        foreach ($similarities as $productIdKey => $similarity) {
            ...
            $products = array_filter($this->products, function ($product) use ($id) { return $product->tripID === $id; });
            if (! count($products)) {
                continue;
            }
    ...
    protected function calculateSimilarityScore($productA, $productB)
    {
        ...
        return array_sum([
                ...
                (Similarity::euclidean(
                        ...
                        Similarity::minMaxNorm([$productA->priceForSeat], 0, $this->priceHighRange),
                        Similarity::minMaxNorm([$productB->priceForSeat], 0, $this->priceHighRange)
                    ) * $this->priceWeight),

                (Similarity::jaccard($productA->startStation, $productB->startStation) * $this->categoryWeight)
